...
IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = N'FK_parts_supplier')
...
SELECT s.id AS supplier_id, s.shop_id,
    s.opening_balance + ISNULL(t.alis_total, 0) - ISNULL(t.odeme_total, 0) - ISNULL(t.iade_total, 0) AS balance,
...
BEGIN SET NOCOUNT ON;
    ...
    BEGIN TRY BEGIN TRAN;
        ...
        IF @source = N'disaridan' AND @supplier_id IS NOT NULL BEGIN
            ...
            VALUES (NEWID(),@shop_id,@supplier_id,N'alis',ISNULL(@purchase_price,0)*@quantity,@new_part_id,N'İş emri için dışarıdan alım: '+@name,@created_by);
        END
        COMMIT;
...
BEGIN SET NOCOUNT ON;
    ...
    BEGIN TRY BEGIN TRAN;
        ...
        INSERT INTO dbo.supplier_transactions (id,shop_id,supplier_id,type,amount,work_order_part_id,description,created_by)
        VALUES (NEWID(),@shop_id,@supplier_id,N'iade',ISNULL(@purchase_price,0)*@quantity,@work_order_part_id,N'Tedarikçiye iade: '+@name,@created_by);
        COMMIT;
    END TRY BEGIN CATCH IF @@TRANCOUNT>0 ROLLBACK; THROW; END CATCH
END
GO
